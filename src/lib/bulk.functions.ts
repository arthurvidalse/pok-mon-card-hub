...
export const listBulkSets = createServerFn({ method: "GET" }).handler(
  async (): Promise<BulkSetSummary[]> => {
    ...
    for (const row of data ?? []) {
      ...
      if (row.set_name) current.setName = row.set_name;
    ...
    return [...grouped.values()].sort((a, b) => a.setName.localeCompare(b.setName));
  },
);

export const getBulkSetGallery = createServerFn({ method: "GET" })
  .inputValidator((input: { setId: string }) => input)
  .handler(async ({ data }): Promise<BulkGalleryCard[]> => {
    const { createPublicClient } = await import("./supabase-public.server");
    const supabase = createPublicClient();
    ...
    for (const rule of rules || []) {
      rulesMap.set(`${rule.rarity}__${rule.condition}`, Number(rule.price ?? 0));
    }

    // Group stock by normalized local id
    const stockByLocal = new Map<string, BulkStockEntry[]>();
    for (const row of (rows ?? []) as BulkCardRow[]) {
      if (Number(row.quantity) <= 0) continue;
      const key = normalizeLocalId(row.local_id);
      const priceRule = row.rarity ? rulesMap.get(`${row.rarity}__${row.condition}`) : undefined;
      const price =
    ...
    // Try to enrich with tcgdex set/card data. If unavailable, fallback to stock rows.
    try {
      ...
      // Fallback: build gallery from rows
      const byLocal = new Map<string, BulkGalleryCard>();
      for (const row of (rows ?? []) as BulkCardRow[]) {
        if (Number(row.quantity) <= 0) continue;
        const local = row.local_id;
        const entry = byLocal.get(local) ?? {
          localId: local,
          name: row.card_name ?? `#${local}`,
          image: row.image_url,
          rarity: row.rarity,
          stock: stockByLocal.get(normalizeLocalId(local)) ?? [],
        };
